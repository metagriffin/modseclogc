...
class TestRecord(unittest.TestCase):
  ...
  def test_record(self):
    rec = record.Record('7db5817a')
    rec.append(part.Part('7db5817a', 'A', '''\
[18/Mar/2016:18:39:17 --0400] VuyDlX8AAQEAAG5ffLsAAAAA 127.0.0.1 33184 127.0.0.1 80
'''))
    rec.append(part.Part('7db5817a', 'B', '''\
...
'''))
    rec.append(part.Part('7db5817a', 'F', '''\
HTTP/1.1 404 Not Found
Content-Length: 288
Content-Type: text/html; charset=iso-8859-1

'''))
    rec.append(part.Part('7db5817a', 'E', '''\
...
'''))
    rec.append(part.Part('7db5817a', 'H', '''\
...
'''))
    rec.append(part.Part('7db5817a', 'Z', ''))

    self.assertEqual(rec.id, '7db5817a')
    self.assertEqual(rec.parts['A'].timestamp, 1458340757)
    self.assertEqual(rec.parts['A'].unique_id, 'VuyDlX8AAQEAAG5ffLsAAAAA')
